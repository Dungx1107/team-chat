...
async function openProfileModal() {
  ...
  openModal(`
    <div class="p-5">
      <h3 class="font-bold text-lg text-slate-900 dark:text-slate-100 mb-4">Hồ sơ của tôi</h3>

      <div class="flex items-center gap-4 mb-5">
        <div id="profile-avatar-preview">${renderAvatar(user, 64)}</div>
        <div class="min-w-0 flex-1">
          <button onclick="document.getElementById('avatar-input').click()"
                  class="px-3 py-1.5 text-xs font-medium border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition">
            Đổi ảnh đại diện
          </button>
          <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-1.5">JPEG, PNG, GIF, WEBP · tối đa 5MB</p>
      ...
      <form onsubmit="onSaveProfile(event)" class="space-y-3.5">
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Họ</label>
          ...
          <div>
            <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Tên</label>
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Trạng thái</label>
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Giới thiệu</label>
          <textarea id="profile-bio" maxlength="500" rows="3"
                    class="w-full px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500 resize-none scroll-thin"
                    placeholder="Đôi dòng về bạn...">${escapeHtml(user.bio || "")}</textarea>
        </div>

        <div class="text-[11px] text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 space-y-0.5">
          <div>Tên đăng nhập: <span class="font-mono text-slate-700 dark:text-slate-300">@${escapeHtml(user.username)}</span></div>
          <div>Email: <span class="font-mono text-slate-700 dark:text-slate-300">${escapeHtml(user.email)}</span></div>
          <div>Tham gia: ${new Date(user.created_at).toLocaleDateString("vi-VN")}</div>
        </div>

        <div class="flex gap-2 pt-1">
          <button type="button" onclick="closeModal()"
                  class="flex-1 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800">Đóng</button>
          <button type="submit"
...
async function onAvatarSelected(event) {
  ...
  preview.innerHTML = `<div class="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] text-slate-400 dark:text-slate-500">Đang tải...</div>`;
    ...
    openModal(`
      <div class="p-5">
        <div class="flex items-center gap-4 mb-4">
          ...
          <div class="min-w-0">
            <div class="font-bold text-lg text-slate-900 dark:text-slate-100 truncate">${escapeHtml(user.full_name)}</div>
              <div class="text-xs text-slate-500 dark:text-slate-400">@${escapeHtml(user.username)}</div>
            ${user.status ? `<div class="text-xs text-emerald-600 mt-1">${escapeHtml(user.status)}</div>` : ""}
          </div>
        </div>
        ${
          user.bio
            ? `<div class="text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 leading-relaxed mb-4">${escapeHtml(user.bio)}</div>`
            : `<p class="text-xs text-slate-400 dark:text-slate-500 italic mb-4">Người này chưa viết giới thiệu.</p>`
        }
        <button onclick="closeModal()"
                class="w-full py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800">Đóng</button>
      </div>
    `);
  } catch (err) {
    toast(err.message, "error");
